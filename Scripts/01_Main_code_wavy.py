...
b     = 6.0                 #Interlayer wave hight
h     = 4.0                 #Interlayer wave width

rw    = 1.4                 #Interlayer wave radios

x0    = 0.0                 #Initial x position of wave 
y0    = 6.2                 #Initial y position of wave 
...
# --- Auto-save a copy of this script (named based on model_name) ---

script = inspect.getsourcefile(lambda: None)
with open(model_name + "_script_backup.py", "w") as f:
...
# Create Part
def get_hw_angle(h,b,R):
    # calculate h0 from the h without the radius R with width b
    # https://www.wolframalpha.com/input?i=solve%28z%3D%3Dx%2B2*R*%281%2Fcos%28arctan%28z*2%2Fb%29%29-1%29%2Cz%29
    return ( (-2*b**2*R + b**2*h + 2* (b**4*R**2 - 16*b**2*R**3*h + 4*b**2*R**2*h**2)**0.5 ) / (b**2 - 16*R**2) )

# this function has been provide to  create wavy interlyer due to input geometries 
def draw_waves(s,b,h,t,rw,x0,y0,x_r,all_points,centers):
    y0_modified = rw-(t/2) + y0      # Initial y position
    y0_at_x0=y0-(rw-(t/2))

    # for h=0: only draw two flat lines
    if h == 0.:
        s.Line(point1=(x_r[0]-2,y0_modified-t/2.),point2=(x_r[1]+2,y0_modified-t/2.))
        s.Line(point1=(x_r[0]-2,y0_modified+t/2.),point2=(x_r[1]+2,y0_modified+t/2.))
        return
    
    # draw the waves as lines and arcs
    n_negative = -int((x_r[0]+x0)/b)+1
    n_positive = int((x_r[1]+x0)/b)+1

    # list of position of single waves
    pos_list = [-i*b+x0 for i in range(n_negative+1)]+[i*b+x0 for i in range(n_positive)] 
    pos_list = list(dict.fromkeys(pos_list)) # remove duplicates

    # adapt h0 (height of the triangle wave) according to rw
    h0 = get_hw_angle(h,b,rw)

    # new version: middle line and then offset
    # --------------------------------------------------
    phi_waves = np.arctan(h0*2/b)
    dx = rw*np.sin(phi_waves)
    dy = dx*np.tan(phi_waves)
    y_cent = h0/2-rw/np.cos(phi_waves)
    dx_offs = t/2 * np.sin(phi_waves)
    dy_offs = t/2 * np.cos(phi_waves)
    # --------------------------------------------------
    for pos_i in pos_list:
        # if waves are in x-direction
        if x_r != ():
            # draw the lines
            # left
            point1=(dx-dx_offs+pos_i,-h0/2+dy+dy_offs+y0_modified)
            point2=(b/2-dx-dx_offs+pos_i,h0/2.-dy+dy_offs+y0_modified)
            ...
            s.Line(point1=point1 ,point2=point2)
            # right
            point1=(b/2+dx+dx_offs+pos_i,h0/2.-dy+dy_offs+y0_modified)
            point2=(b-dx+dx_offs+pos_i,-h0/2+dy+dy_offs+y0_modified)
            s.Line(point1=point1,point2=point2)
            point1=(b/2+dx-dx_offs+pos_i,h0/2.-dy-dy_offs+y0_modified)
            point2=(b-dx-dx_offs+pos_i,-h0/2+dy-dy_offs+y0_modified)
            all_points.append(point1)
            ...
            # left arc
            point1=(dx-dx_offs+pos_i,-h0/2+dy+dy_offs+y0_modified)
            point2=(-dx+dx_offs+pos_i,-h0/2+dy+dy_offs+y0_modified)
            center = (pos_i, -y_cent+y0_modified)
            centers.append(center)
            s.ArcByCenterEnds(center=center, point1=point1,
                              point2=point2, direction=CLOCKWISE)
            point1=(dx+dx_offs+pos_i,-h0/2+dy-dy_offs+y0_modified)
            point2=(-dx-dx_offs+pos_i,-h0/2+dy-dy_offs+y0_modified)
            all_points.append(point1)
            all_points.append(point2)
            ...
            point1=(b/2-dx-dx_offs+pos_i,h0/2.-dy+dy_offs+y0_modified)
            point2=(b/2+dx+dx_offs+pos_i,h0/2.-dy+dy_offs+y0_modified)
            center = (b/2.+pos_i, y_cent+y0_modified)
            centers.append(center) 
            s.ArcByCenterEnds(center=center, point1=point1,
                              point2=point2, direction=CLOCKWISE)
            point1=(b/2-dx+dx_offs+pos_i,h0/2.-dy-dy_offs+y0_modified)
...
# The function is provided to plot image from the result to check the results during the simulation
...
# the folloing function is for calling Matrix Maximum Principle stress in the matrix
...
# Creat parts
# Create the sketch for base_shell
...
# Add partition with wave drawing
s = model.ConstrainedSketch(name='wave_p', sheetSize=200)
all_points = []
centers =[]
draw_waves(s,b,h,t,rw,x0,y0,x_r,all_points,centers)
# Partition the face using the created sketch
p.PartitionFaceBySketch(faces=p.faces[:],sketch=s)
# Create sets
p.Set           (name='all'         , faces=p.faces[:])
p.Set           (name='all_xsym'    , edges=p.edges.getByBoundingBox(xMax=TOL))
p.Set           (name='eg_xsym2'    , edges=p.edges.findAt(((0.0, n_h+TOL, 0.0),)))
p.Set           (name='eg_cs'       , edges=p.edges.findAt(((b/2, 9.6, 0.0),)))
p.SetByBoolean  (name='eg_xsym1'        , operation=DIFFERENCE, sets=(p.sets['all_xsym'], p.sets['eg_xsym2']))
...
all_points = [i for i in all_points if i[0] >= 0 and i[0] <= s_l/2]                 # remove out of border nodes
# all_points = [(round(x, 4), round(y, 4)) for x, y in all_points]                    # round the number                
all_points = list(dict.fromkeys(all_points))                                        # remove duplicates
all_points  = sorted(all_points, key=lambda point: point[0])                        # Sort nodes based on x-coordinate
all_points = [(x, y, 0.0) for x, y in all_points]
edge_points = tuple((pt,) for pt in all_points)
p.Set( name='eg_dlm'              , edges=p.edges.findAt(*edge_points))
...
asmb.translate(instanceList=('up_rol-1', ), 
    vector=(0.0, s_h, 0.0))
ins1=model.rootAssembly.instances['base_shell-1']
ins2=model.rootAssembly.instances['up_rol-1']
ins3=model.rootAssembly.instances['btm_rol-1']
...
# asmb.setMeshControls(algorithm=MEDIAL_AXIS, regions=ins1.faces[:])
asmb.seedEdgeBySize(constraint=FINER, edges=ins1.sets['eg_cs'].edges, size=m_s/4)
...
model.ContactStd(createStepName='Initial', name='general')
int3=model.interactions['general']
